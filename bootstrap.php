...
if (is_admin()) {
    ...
    if (WP_DEBUG) {
        $router->addRoute(new Route('nm_config', ConfigurationController::class, 'list'))
            ...
            ->addRoute(new Route('nm_config_select_language', ConfigurationController::class, 'selectLanguage', true))
        ;
    }
    ...
    if (WP_DEBUG) {
        $coreAdminPlugin
            ...
            ->addResource(
                new Resource(Resource::TYPE_CSS, CoreModule::instance()->getPluginUrl() . 'css/backend/bundle.css')
            )
            ->addResource(
                new Resource(Resource::TYPE_JS, CoreModule::instance()->getPluginUrl() . 'js/backend/bundle.js')
            );
    } else {
        $coreAdminPlugin
            ->addResource(
                new Resource(Resource::TYPE_CSS, CoreModule::instance()->getPluginUrl() . 'css/backend/bundle.min.css')
            )
            ->addResource(
                new Resource(
                    Resource::TYPE_CSS,
            ...
            ->addResource(
                new Resource(Resource::TYPE_JS, CoreModule::instance()->getPluginUrl() . 'js/backend/bundle.min.js')
            );
    }

    // Define menu links
    $mainLink = new MenuLink('NiceModules Core', 'Config', 'nm_index');
    ...
    if (WP_DEBUG) {
        $coreAdminPlugin
            ->addMenuLink(new MenuLink('Configuration values', 'Configuration values', 'nm_config', $mainLink))
